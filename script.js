...
window.onpointerdown = (e)=> {
  gsap.timeline({defaults:{duration:0.3, ease:'back.out(4)'}})
    ...
    .add(()=>{ //check distance between obj and dagger
...
function addobj(){ 
  ...
  stageBg.append(f)
  objTLs.push(
    gsap.timeline({onComplete:()=>{f.remove(); objTLs.shift()}})
    .fromTo(f, {
      innerHTML:(objNum==8||objNum==36)?'⏱️':obj[gsap.utils.random(0,obj.length-1,1)],
      fontSize:99,
      xPercent:-50,
      yPercent:-50,
      y:innerHeight+99,
      x:gsap.utils.random(200,innerWidth-100,1),
      rotate:(objNum%2==0)?10:-10,
...
const objTL = gsap.to(window, {duration:1, repeat:50, onRepeat:addobj})

const timerTL = gsap.timeline({onComplete:gameEnd})
  .to('.timer .face', {rotate:50, ease:'power1.in'})
  .to('.timer .face', {rotate:0, ease:'none', duration:objTL.totalDuration()})
